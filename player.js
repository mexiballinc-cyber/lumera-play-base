...
export function renderPlayer(container, { item, selectedEp = null, title = '' }) {
  ...
  const existing = document.getElementById('playerWrapper');
  if (existing) existing.remove();

  const wrapper = document.createElement('div');
  wrapper.id = 'playerWrapper';
  wrapper.style.cssText = `position: fixed; inset: 0; background: black; z-index: 10000; display: flex; flex-direction: column; justify-content: space-between; font-family: sans-serif; color: white;`;
  ...
  wrapper.innerHTML = `
    ...
    <div style="padding: 20px; display: flex; justify-content: space-between; align-items: center; background: linear-gradient(to bottom, rgba(0,0,0,0.8), transparent); z-index: 10;">
      <div style="display: flex; align-items: center; gap: 15px;">
        ...
        <span style="font-size: 16px; font-weight: 600;">${title || item.title || ''}</span>
      </div>

      <!-- BOTÓN DE SUBTÍTULOS E IDIOMAS -->
      <div style="position: relative;">
        ...
        <div id="langDropdown" style="display: none; position: absolute; right: 0; top: 40px; background: #111; border: 1px solid #333; border-radius: 8px; width: 260px; padding: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.8); z-index: 20;">
          <p style="margin: 0 0 6px 0; font-size: 11px; color: #888; text-transform: uppercase;">Audio / Doblaje</p>
    ...
    <!-- VIDEO -->
    <video id="customVideo" src="${videoSrc}" style="width: 100%; height: 100%; object-fit: contain;" autoplay></video>
  ...
  const video = wrapper.querySelector('#customVideo');
  const btnPlayPause = wrapper.querySelector('#btnPlayPause');
  ...
  // Eventos de control
  wrapper.querySelector('#btnBackPlayer').onclick = () => {
    video.pause();
    wrapper.remove();
  };
  ...
  wrapper.querySelector('#btnFullscreen').onclick = () => {
    if (!document.fullscreenElement) wrapper.requestFullscreen();
    else document.exitFullscreen();
  };

  wrapper.querySelector('#btnAudioSubMenu').onclick = () => {
    dropdown.style.display = dropdown.style.display === 'block' ? 'none' : 'block';
  };

  // Listar Doblajes
  const dubList = wrapper.querySelector('#dubList');
  const langNames = { es: 'Español', en: 'Inglés', de: 'Alemán', pt: 'Portugués', fr: 'Francés', ja: 'Japonés' };
  
  dubList.innerHTML = `<button class="opt-btn" data-url="${videoSrc}" style="text-align: left; background: #222; border: none; color: white; padding: 6px; border-radius: 4px; cursor: pointer; font-size: 12px;">Original / Principal</button>`;
  ...
  // Listar Subtítulos
  const subList = wrapper.querySelector('#subList');
