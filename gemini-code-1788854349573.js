// auth.js - Flujo Lumera con 6 Idiomas, Perfiles Redondos, Modal Glassmorphism y Drawer
...
// SELECCIÓN DE PERFILES (DESPLEGABLE DE 6 IDIOMAS Y DISEÑO RESPONSIVE)
async function renderProfileSelection(container) {
  ...
  let html = `
    <div style="padding: 20px 10px; text-align: center; max-width: 800px; margin: 10px auto;">
      ...
      <!-- HEADER CON IDIOMA, TÍTULO Y LÁPIZ -->
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; gap: 10px;">
        
        <!-- SELECTOR DE 6 IDIOMAS -->
        <select id="selectLanguageGlobal" style="background: rgba(20,20,20,0.8); border: 1px solid #d4af37; color: #d4af37; padding: 6px 10px; border-radius: 8px; font-weight: bold; cursor: pointer;">
        ...
        <h1 style="color: #fff; font-size: 1.6rem; margin: 0; flex-grow: 1;">${t.whoIsWatching}</h1>

        <button id="btnEditarPerfilesMode" title="${t.editProfiles}" style="background: transparent; border: 1px solid #d4af37; color: #d4af37; border-radius: 50%; width: 38px; height: 38px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
  ...
  perfiles.forEach(p => {
    ...
    html += `
      <div class="card-perfil-item" data-id="${p.id}" data-kids="${p.isKids}" style="cursor: pointer; text-align: center; position: relative;">
        ...
        <button class="btn-edit-single-profile" data-json='${JSON.stringify(p)}' style="display:none; position:absolute; top:-5px; right:-5px; background:rgba(0,0,0,0.9); border:1px solid #d4af37; color:#d4af37; border-radius:50%; width:32px; height:32px; cursor:pointer;">✎</button>
      </div>
    `;
  });
  ...
  // Cambio de Idioma dinámico al seleccionar
  document.getElementById('selectLanguageGlobal').onchange = (e) => {
...
// MODAL PARA CREAR / EDITAR / BORRAR PERFIL
...
// FEED CON HERO DE 10 SEGUNDOS Y FILAS POR CATEGORÍA
...
// CERRAR EL DRAWER CON CLIC AL OVERLAY VACÍO
function conectarMenuDrawer() {
  ...
  if (overlay) overlay.onclick = cerrarDrawer;
