...
// DICCIONARIO COMPLETO CON LOS 6 IDIOMAS
const i18n = {
  es: {
...
// INICIALIZACIÓN Y SUSCRIPCIÓN A FIREBASE
...
function renderAuthScreen(container) {
  ...
  container.innerHTML = `
    <div style="max-width: 400px; margin: 60px auto; background: rgba(20,20,20,0.9); padding: 30px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); text-align: center; backdrop-filter: blur(12px);">
...
// SELECCIÓN DE PERFILES (EXPORTACIÓN CORREGIDA)
  ...
  perfiles.forEach(p => {
    ...
    html += `
      <div class="card-perfil-item" data-id="${p.id}" data-kids="${p.isKids}" style="cursor: pointer; text-align: center; position: relative;">
        <img src="${p.avatar}" class="${avatarClass}" style="width: 105px; height: 105px; border-radius: 50%; object-fit: cover; ${staticStyle} transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
    ...
    if (todosLosContenidos.length === 0) {
      ...
      Object.keys(agrupados).forEach(catNombre => {
        ...
        agrupados[catNombre].forEach(item => {
          mainHtml += `
            <div class="card-item-media" data-json='${JSON.stringify(item)}' style="flex: 0 0 140px; background: rgba(255,255,255,0.05); border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.04)'" onmouseout="this.style.transform='scale(1)'">
...
function reproducirContenido(item) {
  ...
  container.innerHTML = `
    <div style="max-width: 900px; margin: 0 auto; padding: 10px; color: white;">
      <button id="btnVolverFeed" style="background: transparent; border: 1px solid #d4af37; color: #d4af37; padding: 8px 16px; border-radius: 8px; cursor: pointer; margin-bottom: 20px;">
        ← Volver al catálogo
      </button>
      ...
      <div style="margin-top: 20px;">
        ...
        <p style="color: #d4af37; font-size: 14px; font-weight: bold;">${(item.type || 'CONTENIDO').toUpperCase()} • ${item.category || 'General'}</p>
...
// CONTROL DEL MENÚ LATERAL Y OVERLAY
function conectarMenuDrawer() {
  const drawer = document.querySelector('.glass-drawer') || document.getElementById('drawer');
  const overlay = document.querySelector('.overlay') || document.getElementById('overlay');
  const btnMenu = document.querySelector('.btn-menu, #btnMenu, .menu-toggle, .glass-nav .svg-btn');
  ...
  const drawerLinks = document.querySelectorAll('.drawer-links .nav-item, #drawer .nav-item');
  ...
  drawerLinks.forEach((link) => {
    link.onclick = (e) => {
      ...
      if (key === 'home' || text.includes('inicio') || text.includes('home')) {
        entrarPlataforma({ isKids: currentPerfilKids, filtroTipo: 'todos' });
      } else if (key === 'movies' || text.includes('película') || text.includes('movies') || text.includes('films')) {
        entrarPlataforma({ isKids: currentPerfilKids, filtroTipo: 'pelicula' });
      } else if (key === 'series' || text.includes('serie') || text.includes('series')) {
        entrarPlataforma({ isKids: currentPerfilKids, filtroTipo: 'serie' });
      } else if (key === 'kids' || text.includes('niño') || text.includes('kids') || text.includes('enfants')) {
        entrarPlataforma({ isKids: true, filtroTipo: 'todos' });
      } else if (key === 'profiles' || text.includes('perfil') || text.includes('profile')) {
        renderProfileSelection(document.getElementById('appContainer'));
      }
    };
  });
