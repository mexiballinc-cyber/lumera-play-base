// auth.js - Flujo de Autenticación, Perfiles, Descripción de Contenido y Reproductor
import { cerrarDrawerGlobal } from './app.js';
import { auth, db, signInWithEmailAndPassword, createUserWithEmailAndPassword, onAuthStateChanged, signOut, collection, getDocs, addDoc, doc, deleteDoc, getDoc } from './firebase.js';
...
export async function entrarPlataforma({ isKids = false, filtroTipo = 'todos' } = {}) {
  ...
  try {
    ...
    if (todosLosContenidos.length === 0) {
      ...
      Object.keys(agrupados).forEach(catNombre => {
        ...
        agrupados[catNombre].forEach(item => {
          mainHtml += `
            <div class="card-contenido-item" data-id="${item.id}" style="flex: 0 0 140px; background: rgba(255,255,255,0.05); border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.04)'" onmouseout="this.style.transform='scale(1)'">
    ...
    document.querySelectorAll('.card-contenido-item').forEach(card => {
      card.onclick = () => {
        const id = card.getAttribute('data-id');
        abrirPantallaDescripcion(id);
      };
    });
...
// INTERFAZ DE DESCRIPCIÓN DEL CONTENIDO
async function abrirPantallaDescripcion(id) {
  const t = i18n[currentLang];
  ...
  const modal = document.createElement('div');
  modal.style.cssText = "position:fixed; inset:0; background:rgba(10,10,10,0.95); z-index:900; overflow-y:auto; backdrop-filter:blur(10px); color:white;";

  let episodiosHtml = '';
  if (item.type === 'serie' && item.seasons && item.seasons.length > 0) {
    episodiosHtml += `
      <div style="margin-top:25px;">
        <h3 style="color:#d4af37; font-size:1.2rem; margin-bottom:12px;">${t.season} 1 - ${t.episodes}</h3>
        <div style="display:flex; flex-direction:column; gap:8px;">
    `;
    item.seasons[0].episodes.forEach((ep, idx) => {
      episodiosHtml += `
        <div class="ep-row-item" data-video="${ep.videoUrl}" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.05); padding:12px 16px; border-radius:8px; border:1px solid rgba(255,255,255,0.1); cursor:pointer;">
          <span style="font-size:14px; font-weight:bold;">${idx + 1}. ${ep.title}</span>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="#d4af37"><path d="M8 5v14l11-7z"/></svg>
        </div>
      `;
    });
    episodiosHtml += `</div></div>`;
  }

  modal.innerHTML = `
    <div style="max-width:800px; margin:0 auto; padding-bottom:40px; position:relative;">
      <button id="btnCloseDesc" style="position:absolute; top:20px; right:20px; background:rgba(0,0,0,0.6); border:1px solid rgba(255,255,255,0.2); color:white; border-radius:50%; width:40px; height:40px; cursor:pointer; z-index:10; font-size:18px;">✕</button>
      ...
      <div style="padding:0 25px; margin-top:-40px; position:relative; z-index:2;">
        <h1 style="font-size:2.2rem; color:#fff; font-weight:bold; margin-bottom:10px;">${item.title}</h1>
        <p style="color:#aaa; font-size:14px; line-height:1.5; margin-bottom:20px;">${item.description || ''}</p>

        <button id="btnPlayMain" style="display:inline-flex; align-items:center; gap:10px; padding:12px 28px; background:#d4af37; border:none; color:black; font-weight:bold; font-size:16px; border-radius:30px; cursor:pointer; transition:transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
  ...
  document.getElementById('btnCloseDesc').onclick = () => modal.remove();

  document.getElementById('btnPlayMain').onclick = () => {
    let videoUrl = item.videoUrl;
    if (item.type === 'serie' && item.seasons && item.seasons[0] && item.seasons[0].episodes[0]) {
      videoUrl = item.seasons[0].episodes[0].videoUrl;
    }
    abrirReproductorSpark(videoUrl, item.subtitles, item.audios);
  };

  modal.querySelectorAll('.ep-row-item').forEach(epRow => {
    epRow.onclick = () => {
      const videoUrl = epRow.getAttribute('data-video');
      abrirReproductorSpark(videoUrl, item.subtitles, item.audios);
    };
  });
}

// REPRODUCTOR PERSONALIZADO SPARK
function abrirReproductorSpark(videoUrl, subtitles = {}, audios = {}) {
  const playerModal = document.createElement('div');
  playerModal.style.cssText = "position:fixed; inset:0; background:black; z-index:1000; display:flex; align-items:center; justify-content:center;";

  let subTracks = `<track kind="subtitles" label="Desactivados" srclang="off" default>`;
  Object.keys(subtitles).forEach(lang => {
    subTracks += `<track kind="subtitles" label="${lang.toUpperCase()}" srclang="${lang}" src="${subtitles[lang]}">`;
  });

  playerModal.innerHTML = `
    <button id="btnClosePlayer" style="position:absolute; top:20px; right:20px; background:rgba(255,255,255,0.2); border:none; color:white; border-radius:50%; width:40px; height:40px; cursor:pointer; z-index:1010; font-size:20px;">✕</button>
    <video id="lumeraVideoPlayer" controls autoplay style="width:100%; height:100%; object-fit:contain;">
  ...
  document.getElementById('btnClosePlayer').onclick = () => playerModal.remove();
}
