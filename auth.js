// auth.js - Conexión total con tu CSS
...
async function renderProfileSelection(container) {
  ...
  container.innerHTML = `<h2 style="color:var(--gold-accent); text-align:center; margin-top:50px;">${t.loading}</h2>`;
  ...
  let html = `
    <div style="padding: 20px 10px; text-align: center; max-width: 800px; margin: 0 auto;">
      <div style="display: flex; align-items: center; justify-content: center; gap: 15px; margin-bottom: 30px;">
        ...
        <button id="btnEditarPerfilesMode" class="svg-btn" title="${t.editProfiles}">
  ...
  perfiles.forEach(p => {
    // Usa la clase CSS exactamente definida en tu CSS
...
// MODAL APLICANDO TU CLASE .glass-modal
async function abrirModalGestionPerfil(perfilExistente = null) {
  ...
  modal.innerHTML = `
    <div class="glass-modal" style="padding:25px; border-radius:20px; width:100%; max-width:400px; color:white;">
      <h3 style="color:var(--gold-accent); margin-bottom:20px; text-align:center;">${perfilExistente ? t.editProfiles : t.createProfile}</h3>
      ...
      <div style="display:flex; flex-direction:column; gap:15px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-top:10px;">
          ...
          <div style="display:flex; gap:10px;">
            ...
            <button id="btnSaveProf" type="button" style="padding:10px 20px; background:var(--gold-accent); border:none; color:black; font-weight:bold; border-radius:8px; cursor:pointer;">${t.save}</button>
          </div>
        </div>
...
// CATÁLOGO APLICANDO TU CLASE .fila-scroll
export async function entrarPlataforma({ isKids = false, filtroTipo = 'todos' } = {}) {
  currentPerfilKids = isKids;
  const t = i18n[currentLang] || i18n.es;
  const container = document.getElementById('appContainer');
  container.innerHTML = `<h2 style="color:var(--gold-accent); text-align:center; margin-top:40px;">${t.loading}</h2>`;
      ...
      Object.keys(agrupados).forEach(catNombre => {
        mainHtml += `
          <div style="margin-bottom: 30px;">
            ...
            <!-- Usamos tu clase de CSS exacto para el scroll -->
...
// CONEXIÓN EXACTA CON TUS CLASES CSS .open Y .active
