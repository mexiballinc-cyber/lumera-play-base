...
export function renderAdminPanel(container) {
  ...
  document.getElementById('btnNuevoContenido').onclick = () => abrirModalContenido();

  // Guardar Hero y Avatares
...
// MODAL CONTENIDO (DINÁMICO PELÍCULA / SERIE)
async function abrirModalContenido() {
  ...
  catSnapshot.forEach(doc => {
    catOptions += `<option value="${doc.data().name}">${doc.data().name}</option>`;
  });
  ...
  modal.innerHTML = `
    <div style="background:#151515; padding:25px; border-radius:16px; border:1px solid #d4af37; width:90%; max-width:650px; color:white; margin:auto;">
      <h3 style="color:#d4af37; margin-bottom:15px;">Añadir Contenido</h3>
      <form id="formModalContenido" style="display:flex; flex-direction:column; gap:12px;">
        ...
        <input type="url" id="cntBanner" placeholder="URL Banner / Foto Descripción (Imgur)" required style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
        <textarea id="cntDesc" placeholder="Descripción breve" rows="2" style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;"></textarea>
        ...
        <!-- SECCIÓN PELÍCULA -->
        <div id="seccionPeliculaForm" style="display:flex; flex-direction:column; gap:10px; border-top:1px solid #333; padding-top:10px;">
          <input type="url" id="cntVideoUrl" placeholder="URL Video Película (Internet Archive MP4)" style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
        </div>

        <!-- SECCIÓN SERIE -->
        <div id="seccionSerieForm" style="display:none; flex-direction:column; gap:10px; border-top:1px solid #333; padding-top:10px;">
          <h4 style="color:#d4af37; margin:0;">Añadir Episodios (Temporada 1)</h4>
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
            ...
            <input type="url" id="epAudioEs" placeholder="URL Audio Doblaje ES (Opcional)" style="padding:8px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
            <input type="url" id="epSubEs" placeholder="URL Subtítulos ES (.vtt) (Opcional)" style="padding:8px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
          </div>
  ...
  document.getElementById('btnAgregarEpisodio').onclick = () => {
    ...
    if (title && videoUrl) {
      episodiosTemporadaActual.push({ title, videoUrl, audioEs, subEs });
