...
// Instancia de Firestore tomada del contexto global
const db = window.db;
...
export async function renderAdminPanel(container) {
  container.innerHTML = `
    <div style="max-width: 1000px; margin: 0 auto; color: white;">
      <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--glass-border, #333); padding-bottom: 15px; margin-bottom: 20px;">
        <h2 style="color: var(--gold-accent, #d4af37); margin: 0;">Panel de Control Lumera</h2>
        <div style="display: flex; gap: 8px;">
          <button id="tabContents" style="padding: 8px 14px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 6px; cursor: pointer;">Catálogo</button>
  ...
  try {
    ...
    area.innerHTML = `
      <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
        ...
        <button id="btnNewContent" style="padding: 8px 16px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 6px; cursor: pointer;">+ Nuevo Contenido</button>
      </div>
    ...
    grid.innerHTML = adminContents.map(item => `
      <div style="background: #151515; border: 1px solid #282828; border-radius: 8px; padding: 12px; display: flex; flex-direction: column; gap: 8px;">
        ...
        <span style="color: var(--gold-accent, #d4af37); font-size: 11px; text-transform: uppercase;">${item.type}</span>
        <div style="display: flex; gap: 4px; margin-top: auto; flex-wrap: wrap;">
          ...
          ${item.type === 'pelicula' ? `<button class="btn-movie-opts" data-id="${item.id}" style="flex: 1; padding: 6px; background: #222; border: 1px solid var(--gold-accent, #d4af37); color: var(--gold-accent, #d4af37); border-radius: 4px; font-size: 11px; cursor: pointer;">12 Opciones</button>` : ''}
          ${item.type === 'serie' ? `<button class="btn-episodes" data-id="${item.id}" style="flex: 1; padding: 6px; background: #222; border: 1px solid var(--gold-accent, #d4af37); color: var(--gold-accent, #d4af37); border-radius: 4px; font-size: 11px; cursor: pointer;">Episodios</button>` : ''}
    ...
    grid.querySelectorAll('.btn-del').forEach(b => b.onclick = async () => { 
      if (confirm("¿Eliminar este contenido?")) { 
        try {
          await deleteDoc(doc(db, "contents", b.getAttribute('data-id'))); 
          renderContentsTab(); 
        } catch (e) {
    ...
    area.innerHTML = `<p style="color: #ff4d4d;">Error al cargar el catálogo: ${error.message}</p>`;
  }
}
...
function abrirModalCrearEditarContenido(item = null) {
  ...
  modal.style.cssText = `position: fixed; inset: 0; z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 20px; background: rgba(0,0,0,0.8);`;

  modal.innerHTML = `
    <div style="background: #141414; border: 1px solid var(--gold-accent, #d4af37); border-radius: 12px; width: 100%; max-width: 450px; padding: 20px; color: white;">
      <h3 style="margin-top: 0; color: var(--gold-accent, #d4af37);">${item ? 'Editar Contenido' : 'Nuevo Contenido'}</h3>
      <form id="formContent" style="display: flex; flex-direction: column; gap: 10px;">
        ...
        <div style="display: flex; justify-content: flex-end; gap: 8px; margin-top: 10px;">
          <button type="button" id="btnClose" style="padding: 8px 12px; background: transparent; border: 1px solid #555; color: white; border-radius: 4px; cursor: pointer;">Cancelar</button>
          <button type="submit" style="padding: 8px 12px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 4px; cursor: pointer;">Guardar</button>
        </div>
      </form>
    </div>
  `;
  ...
  document.getElementById('formContent').onsubmit = async (e) => {
    ...
    try {
      ...
      alert("Error guardando el contenido: " + err.message);
    }
  };
}
...
function abrirFormulario12OpcionesPelicula(movie) {
  const modal = document.createElement('div');
  modal.className = 'glass-modal';
  modal.style.cssText = `position: fixed; inset: 0; z-index: 10000; display: flex; justify-content: center; align-items: center; padding: 20px; overflow-y: auto; color: white; background: rgba(0,0,0,0.85);`;

  const subs = movie.subtitles || {};
  const dubs = movie.doblajes || {};

  modal.innerHTML = `
    <div style="background: #141414; border: 1px solid var(--gold-accent, #d4af37); border-radius: 12px; width: 100%; max-width: 650px; padding: 20px;">
      <h3 style="margin-top: 0; color: var(--gold-accent, #d4af37);">Idiomas de Película: ${movie.title}</h3>
      <form id="formMovieOpts" style="display: flex; flex-direction: column; gap: 12px;">
        ...
        <div style="border: 1px solid #333; padding: 10px; border-radius: 6px;">
          <h4 style="margin: 0 0 8px 0; color: var(--gold-accent, #d4af37); font-size: 13px;">6 Enlaces Subtítulos</h4>
        ...
        <div style="border: 1px solid #333; padding: 10px; border-radius: 6px;">
          <h4 style="margin: 0 0 8px 0; color: var(--gold-accent, #d4af37); font-size: 13px;">6 Enlaces Doblajes</h4>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
        ...
        <div style="display: flex; justify-content: flex-end; gap: 8px;">
          <button type="button" id="btnCloseM" style="padding: 8px 12px; background: transparent; border: 1px solid #555; color: white; border-radius: 4px; cursor: pointer;">Cancelar</button>
          <button type="submit" style="padding: 8px 12px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 4px; cursor: pointer;">Guardar Opciones</button>
        </div>
      </form>
    </div>
  `;
  ...
  document.getElementById('formMovieOpts').onsubmit = async (e) => {
    ...
    try {
      ...
      alert("Error guardando opciones de película: " + err.message);
    }
  };
}
...
function abrirModalGestionEpisodios(serie) {
  const modal = document.createElement('div');
  modal.className = 'glass-modal';
  modal.style.cssText = `position: fixed; inset: 0; z-index: 9999; display: flex; justify-content: center; padding: 20px; overflow-y: auto; color: white; background: rgba(0,0,0,0.85);`;

  let seasons = serie.seasons || [];

  const refresh = () => {
    modal.innerHTML = `
      <div style="background: #111; border: 1px solid var(--gold-accent, #d4af37); border-radius: 12px; width: 100%; max-width: 750px; padding: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; padding-bottom: 10px;">
          <h3 style="color: var(--gold-accent, #d4af37); margin: 0;">Episodios: ${serie.title}</h3>
          <button id="btnCloseEp" style="background: transparent; border: none; color: white; font-size: 20px; cursor: pointer;">✕</button>
        </div>
        <button id="btnAddSeason" style="margin: 15px 0; padding: 8px 14px; background: #222; border: 1px solid var(--gold-accent, #d4af37); color: var(--gold-accent, #d4af37); border-radius: 6px; cursor: pointer;">+ Añadir Temporada</button>
          ...
          ${seasons.map((s, sIdx) => `
            <div style="background: #181818; border: 1px solid #333; border-radius: 8px; padding: 12px;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                ...
                <div>
                  <button class="btn-add-ep" data-sidx="${sIdx}" style="padding: 4px 8px; background: var(--gold-accent, #d4af37); border: none; color: black; border-radius: 4px; font-size: 11px; cursor: pointer;">+ Episodio</button>
...
function abrirFormularioEpisodio(serieId, seasons, sIdx, eIdx = null, callbackRefresh) {
  const modal = document.createElement('div');
  modal.className = 'glass-modal';
  modal.style.cssText = `position: fixed; inset: 0; z-index: 10000; display: flex; justify-content: center; align-items: center; padding: 20px; color: white; background: rgba(0,0,0,0.85);`;
  ...
  modal.innerHTML = `
    <div style="background: #141414; border: 1px solid var(--gold-accent, #d4af37); border-radius: 12px; width: 100%; max-width: 650px; padding: 20px; max-height: 90vh; overflow-y: auto;">
      <h3 style="margin-top: 0; color: var(--gold-accent, #d4af37);">${eIdx !== null ? 'Editar Episodio' : 'Añadir Episodio'}</h3>
      <form id="formEp" style="display: flex; flex-direction: column; gap: 10px;">
        ...
        <div style="display: flex; justify-content: flex-end; gap: 8px;">
          <button type="button" id="btnCloseEpForm" style="padding: 8px 12px; background: transparent; border: 1px solid #555; color: white; border-radius: 4px; cursor: pointer;">Cancelar</button>
          <button type="submit" style="padding: 8px 12px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 4px; cursor: pointer;">Guardar Episodio</button>
        </div>
      </form>
    </div>
  `;
  ...
  document.getElementById('formEp').onsubmit = async (e) => {
    ...
    try {
      ...
      alert("Error actualizando episodio: " + err.message);
    }
  };
}
  ...
  try {
    ...
    area.innerHTML = `
      <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
        ...
        <button id="btnNewAvatar" style="padding: 8px 16px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 6px; cursor: pointer;">+ Añadir Link Imgur</button>
      </div>
    ...
    area.innerHTML = `<p style="color: #ff4d4d;">Error al cargar los avatares: ${err.message}</p>`;
  }
}
  ...
  try {
    ...
    area.innerHTML = `
      <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
        ...
        <button id="btnNewHero" style="padding: 8px 16px; background: var(--gold-accent, #d4af37); border: none; color: black; font-weight: bold; border-radius: 6px; cursor: pointer;">+ Añadir Banner Imgur</button>
      </div>
    ...
    area.innerHTML = `<p style="color: #ff4d4d;">Error al cargar la sección Hero: ${err.message}</p>`;
  }
}
