...
export function renderAdminPanel(container) {
    ...
    <div class="admin-panel" style="position: relative; padding: 25px; color: #fff; max-width: 950px; margin: 20px auto; background: rgba(15, 15, 15, 0.95); backdrop-filter: blur(16px); border-radius: 20px; border: 1px solid rgba(212, 175, 55, 0.3);">
      ...
      <div id="seccion-contenido" class="admin-section">
        ...
        <div id="listaContenidoAdmin" style="display: flex; flex-direction: column; gap: 15px;">
          <p style="color: #aaa;">Cargando catálogo...</p>
        </div>
      </div>
  ...
  // ASIGNAR CLICS A LOS BOTONES DE CREAR
  document.getElementById('btnNuevaCategoria').onclick = (e) => {
    e.preventDefault();
    abrirModalCategoria();
  };

  document.getElementById('btnNuevoContenido').onclick = (e) => {
    e.preventDefault();
    abrirModalContenido();
  };
...
function abrirModalCategoria() {
  ...
  modal.innerHTML = `
    <div style="background:#151515; padding:25px; border-radius:12px; border:1px solid #d4af37; width:320px; color:white;">
      ...
      <input type="text" id="nombreCatInput" placeholder="Nombre (Ej. Acción, Terror)" style="width:100%; padding:10px; margin:15px 0; background:#222; border:1px solid #444; color:white; border-radius:6px; box-sizing:border-box;">
...
function abrirModalContenido() {
  ...
  modal.innerHTML = `
    <div style="background:#151515; padding:25px; border-radius:16px; border:1px solid #d4af37; width:90%; max-width:550px; color:white; margin:auto;">
      ...
      <form id="formModalContenido" style="display:flex; flex-direction:column; gap:12px;">
        <input type="text" id="cntTitulo" placeholder="Título del Contenido" required style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
        ...
        <input type="url" id="cntBanner" placeholder="URL Foto Descripción / Banner (Imgur)" required style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
        <textarea id="cntDesc" placeholder="Descripción breve" rows="3" style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;"></textarea>
        
        <label style="display:flex; align-items:center; gap:10px; color:#ff5555; font-size:14px; cursor:pointer;">
          <input type="checkbox" id="cntMayor7"> Mayor de 7 años (Ocultar en Kids)
        </label>
        ...
        <input type="url" id="cntVideoUrl" placeholder="URL Video Principal (Internet Archive MP4)" required style="padding:10px; background:#222; border:1px solid #444; color:white; border-radius:6px;">
...
// LEER Y RENDERIZAR DESDE FIREBASE
async function cargarListaAdmin() {
  const contenedor = document.getElementById('listaContenidoAdmin');
  if (!contenedor) return;

  contenedor.innerHTML = "<p style='color:#aaa;'>Cargando lista...</p>";
  
  try {
    const querySnapshot = await getDocs(collection(db, "contents"));

    if (querySnapshot.empty) {
      contenedor.innerHTML = "<p style='color:#888;'>No hay nada publicado aún. Presiona '+ Añadir Contenido'.</p>";
      return;
    }
      ...
      card.innerHTML = `
        <div style="display:flex; align-items:center; gap:15px;">
          ...
          <div>
            ...
            <small style="color:#aaa;">${item.type ? item.type.toUpperCase() : 'CONTENIDO'}</small>
          </div>
        </div>
        <div style="display:flex; gap:10px;">
          <button type="button" style="padding:6px 12px; background:transparent; border:1px solid #d4af37; color:#d4af37; border-radius:6px; cursor:pointer;">Editar</button>
          <button type="button" style="padding:6px 12px; background:rgba(255,0,0,0.2); border:1px solid #ff4444; color:#ff4444; border-radius:6px; cursor:pointer;">Borrar</button>
        </div>
      `;
      contenedor.appendChild(card);
    });
  } catch (error) {
    contenedor.innerHTML = "<p style='color:#ff4444;'>Error cargando catálogo.</p>";
  }
}
